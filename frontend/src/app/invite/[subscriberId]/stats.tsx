import {
  ...
  getSubscribersSubscriberIdRankingCounts,
...
export const Stats = async ({ subscriberId }: Props) => {
  const { count: accessCount } = await getSubscribersSubscriberIdRankingClicks(subscriberId)
  const { count: inviteCount } = await getSubscribersSubscriberIdRankingCounts(subscriberId)
  const { position } = await getSubscribersSubscriberIdRankingPosition(subscriberId)
